...
CLOSE_FILE:
    ...
    LDP 0xF9            ; [0x04..0x07] Указатель на TAPE_CMD
    LDI 0x0             ; [0x08..0x09]
    STR                 ; [0x0A] MOTOR = 0
...
WAIT_CMD:
    ...
    LDI 0xD
    SUB B
    JZR DUMP_INIT       ; D -> Короткий прыжок (смещение +2 ниббла)
    
    BOOT                ; Любая другая кнопка -> программный сброс

DUMP_INIT:
    LDP 0xF9
    LDI 0x6             ; MOTOR=1, MODE=1, STRB=0
    STR
...
WRITE_B:
    ...
    ; 2. Проверка состояния мотора (Hardware-Driven Logic)
    PHA                 ; Спасаем введенный код B в стек
    LDP 0xF9            ; Указатель на контроллер ленты
    LDI 0x4
    MOV B, A            ; B = 4 (маска MOTOR)
    LDR
    AND B
    SUB B               ; Если мотор крутится (4), то 4 - 4 = 0 (Z=1)
    JZ IS_ECHO          ; Абсолютный прыжок к хэндшейку ленты
    
    PLA                 ; Если мотор стоит: восстанавливаем баланс стека
ADV_LOOP:
    ; Заинлайненный инкремент адреса (Без CAL)
    LDP 0xF2
    LDR
    INC A
    STR
...
IS_ECHO:
    ; Указатель X:Y уже на 0xF9 после проверки мотора! 
    DEX                 ; X:Y = 0xF8 (TAPE_DAT)
    PLA                 ; Извлекаем введенный ниббл из стека обратно в A
    STR                 ; Пишем в буфер ленты
    
    INX                 ; X:Y = 0xF9 (TAPE_CMD)
    LDI 0x7             ; STRB=1
    STR
    LDI 0x1
    MOV B, A
E_W1:
    LDR
    AND B
    JZR E_W1            ; Ждем ACK=1 (Смещение -5 нибблов)
...
E_W0:
    ...
    AND B
    SUB B
    JZR E_W0            ; Ждем ACK=0 (Смещение -7 нибблов)
    
    JMP ADV_LOOP        ; Возврат к инкременту адреса
